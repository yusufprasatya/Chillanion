                        ...
                        RoundedButton(
                            ...
                            action: {
                                if isRecommendedReminderActive{
                                    ...
                                    let hourBeforeSleep = calendar.date(byAdding: .hour, value: 6, to: bedTime)
                                    
                                    let triggerDateComponents = calendar.dateComponents([.hour, .minute, .second], from: hourBeforeSleep!)
                                    
                                    UserNotificationService.shared.scheduleNotification(identifier: "limitCaffeine", type: "date", timeInterval: nil, title: "Limit Caffeine", body: "Time to cut caffeine for better sleep!☕️🆘", notifHour: triggerDateComponents)
                                ...
                                if isManualReminderActive {
                                    ...
                                    UserNotificationService.shared.scheduleNotification(identifier: "limitCaffeine", type: "date", timeInterval: nil, title: "Limit Caffeine", body: "Time to cut caffeine for better sleep!☕️🆘", notifHour: components)
                                } else if isManualReminderActive {
                                    UserNotificationService.shared.disableNotifications(identifiers: ["limitCaffeine"])
                                    isManual = false
                                }
                                
                                dailyHabitViewModel.updateReminder(name: "Limit Caffeine", isRemind: isRemind, isManual: isManual)
