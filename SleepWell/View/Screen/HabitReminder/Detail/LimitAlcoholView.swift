    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                        ...
                        RoundedButton(
                            ...
                            action: {
                               if isRecommendedReminderActive{
                                ...
                                let hourBeforeSleep = calendar.date(byAdding: .hour, value: 4, to: bedTime)
                                
                                let triggerDateComponents = calendar.dateComponents([.hour, .minute, .second], from: hourBeforeSleep!)
                                
                                UserNotificationService.shared.scheduleNotification(identifier: "limitAlcohol", type: "date", timeInterval: nil, title: "Limit Alcohol", body: "rap up that last sip of alcohol 3-4 hours before bedtime for a cozier night's sleep.", notifHour: triggerDateComponents)
                            } else if isRecommendedReminderActive == false {
                                UserNotificationService.shared.disableNotifications(identifiers: ["limitAlcohol"])
                                isRemind = false
                            }
                            ...
                            if isManualReminderActive {
                                ...
                                UserNotificationService.shared.scheduleNotification(identifier: "limitAlcohol", type: "date", timeInterval: nil, title: "Limit Alcohol", body: "rap up that last sip of alcohol 3-4 hours before bedtime for a cozier night's sleep.", notifHour: components)
                            } else if isManualReminderActive {
                                UserNotificationService.shared.disableNotifications(identifiers: ["limitAlcohol"])
                                isManual = false
                            }
                            
                            dailyHabitViewModel.updateReminder(name: "Limit Alcohol", isRemind: isRemind, isManual: isManual)
                                self
                                    .presentationMode
                                    .wrappedValue
                                    .dismiss()},
                ...
                .onAppear{
                    dailyHabitViewModel.getDailyHabit(name: "Limit Alcohol")
